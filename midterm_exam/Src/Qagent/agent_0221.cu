...
bool  *d_active;
float *d_qtable;
// float2 *d_qmax; // .x action of max qval, .y max qval; size same as 
...
__global__ void Agent_action(int2 *cstate, short *d_action, curandState *d_state, float epsilon, float *d_qtable, bool *d_active) {
	...
	if (d_active[agent_id] == 1) {
		...
		if (rand_state < epsilon) {
			...
			if (action == 4) action = 0; // curand_uniform (0, 1]
		}
		else {
			// exploitation (greedy policy)
			unsigned int qid = (y * COLS + x) * NUM_OF_ACTIONS;
...
short* agent_action(int2* cstate) {
	...
	Agent_action << <1, NUM_OF_AGENTS >> > (cstate, d_action, d_state, epsilon, d_qtable, d_active); 
	return d_action;
}
