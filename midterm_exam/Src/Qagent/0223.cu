...
// <<< #agents, #actions >>>
// <<< 1, (#actions, #agents) >>>
__global__ void Agent_action(int2 *cstate, short *d_action, curandState *d_states, float epsilon, float *d_qtable, bool *d_active, float2 *d_actval)
{
	// unsigned int agent_id = blockIdx.x * blockDim.x + threadIdx.x;
	unsigned int agent_id = blockIdx.y; 
	...
	if (d_active[agent_id] == 1)
	{
		...
		if (rand_state < epsilon) {
			...
			unsigned int aid = threadIdx.x; // action_id
			unsigned int tid = agent_id * ACTIONS + aid;
			action_cache[tid] = (short)threadIdx.x;

			unsigned int q_id = (y * COLS + x) * ACTIONS;
			qval_cache[tid] = d_qtable[q_id + aid];
			...
			#pragma unroll
			while (stride != 0) {
				if (aid < stride && qval_cache[tid] < qval_cache[tid + stride]) {
					// keep larger values in left cache
					qval_cache[tid] = qval_cache[tid + stride];
					action_cache[tid] = action_cache[tid + stride];
				}
				__syncthreads();
				stride /= 2;
			}
			// update: .x action; .y max_qval.
			d_actval[agent_id].x = action_cache[tid/ACTIONS];
			d_actval[agent_id].y = qval_cache[tid/ACTIONS];
...
short* agent_action(int2* cstate) {
	...
	Agent_action <<< 1, (ACTIONS, NUM_OF_AGENTS) >>> (cstate, d_action, d_states, epsilon, d_qtable, d_active, d_actval);
	return d_action;
}
...
// <<< 1, (#actions, #agents) >>>
__global__ void Agent_update(int2* cstate, int2* nstate, float *rewards, float *d_qtable, short *d_action, bool *d_active, float2 *d_actval)
{
	...
	if (d_active[agent_id] == 1) {
		...
		if (rewards[agent_id] == 0) {
			...
			unsigned int x0 = nstate[agent_id].x;
			unsigned int y0 = nstate[agent_id].y;		
			...
			unsigned int q_id = (y0 * COLS + x0) * ACTIONS;
			qval_cache[tid] = d_qtable[q_id + aid];
